...
namespace BeghTools.Core.Services
{
    public class ImageConversionService : ISingletonable
    {
        static public List<string> SupportedFormats = new List<string>() { "png", "jpg", "jpeg", "bmp", "gif", "ico", "webp" };
        public void ConvertImage(string imagePath, string outputPath, string outputFormat, Color backgroundColor = default)
        {
            Bitmap image;
            if (IsWebP(imagePath))
            {
                image = Dynamicweb.WebP.Decoder.Load(imagePath);
            }
            else
            {
            ...
            {
                Dynamicweb.WebP.Encoder.Save(image, outputPath, 75);
            }
